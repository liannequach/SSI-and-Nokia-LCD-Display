...
#define SYSCTL_RCGCSSI_SSI0     0x00000001  // SSI0 Clock Gating Control
...
void Nokia5110_Init(void){
  volatile unsigned long delay;
	SYSCTL_RCGCSSI_R |= SYSCTL_RCGCSSI_SSI0; // activate SSI0 clock
  ...
  SSI0_CC_R = (SSI0_CC_R&~SSI_CC_CS_M)+SSI_CC_CS_SYSPLL;
	...
	// Nokia5110 SCLK requires 0.0 to 4.0 Mbits/s.
  SSI0_CPSR_R = (SSI0_CPSR_R&~SSI_CPSR_CPSDVSR_M)+16;
  SSI0_CR0_R &= ~(SSI_CR0_SCR_M |       // SCR = 0 (3.125 Mbps data rate)
	                // SPH = 0, Data is captured on the first clock edge transition.
                  SSI_CR0_SPH |         
                  // SPO = 0, A steady state Low value is placed on the SSInClk pin 
  ...
  // Set LCD Vop (contrast), which may require some tweaking:
	// try 0xB1 (for 3.3V red SparkFun), 0xB8 (for 3.3V blue SparkFun), 
	// 0xBF if your display is too dark, 
	// or 0x80 to 0xFF if your display is too light
  ...
  // LCD bias mode 1:48: try 0x13 or 0x14. 
	// The Bias setting controls the amount of voltage that goes to the display.
